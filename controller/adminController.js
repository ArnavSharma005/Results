...
export const addSubject = async (req, res) => {
    const { name, code, credits, theoryCredits, practicalCredits, hasPractical, department, year, semester, isCore } = req.body;
...
export const addTeacher = async (req, res) => {
    ...
    try {
        ...
        if (!subjectCodes || !classes || subjectCodes.length !== classes.length) {
            return res.status(400).json({ message: 'Subject codes and classes do not match or are missing.' });
        }
...
export const getSubjectsByTeachers = async(req,res)=>{
    ...
    try{
        ...
        const subjectCodes = teacher.subjectsAndClasses.map(({subjectCode}) => subjectCode);
...
export const getClassesBySubject = async(req,res)=>{
    ...
    try{
        ...
        const subject = teacher.subjectsAndClasses.find(({subjectCode: code}) => code === subjectCode);
        if(!subject){
